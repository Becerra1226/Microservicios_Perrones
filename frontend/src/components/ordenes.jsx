...
function Ordenes() {
  ...
  const [menuModificador, setMenuModificador] = useState({
    id: null,
    tipo: null,
  });
  ...
  const agregarProducto = (producto) => {
    setOrden((ordenActual) => {
      ...
      if (productoExistente) {
        ...
        return [
          ...ordenActual,
          { ...producto, cantidad: 1, extras: [], sin: [] },
        ];
      }
    });
  };
  ...
  const quitarExtra = (idProducto, extraObj) => {
    setOrden((ordenActual) =>
      ordenActual.map((item) => {
        if (item.id === idProducto) {
          const extrasIndex = item.extras.findIndex(
            (e) => e.id === extraObj.id,
          );
          if (extrasIndex > -1) {
  ...
  const calcularSubtotal = () => {
    return orden.reduce((total, item) => {
      ...
      const precioExtras = item.extras.reduce(
        (sum, ext) => sum + Number(ext.precio),
        0,
      );
        ...
        <div className="flex-1 w-full space-y-6">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Selecciona los productos para agregarlos a la cuenta
            </p>
          </div>
          ...
          <div className="p-5 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {orden.length} ítems seleccionados
              </p>
            </div>
          </div>
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex-1 pr-2">
                          ...
                          <span className="text-sm font-medium text-slate-600 block mt-0.5">
                            $
                            {(
                              precioItemConExtras * item.cantidad
                      ...
                      {(item.extras.length > 0 || item.sin.length > 0) && (
                        <div className="flex flex-col gap-1 ml-2 my-1 border-l-2 border-slate-100 pl-3">
                          {extrasVisuales.map((ex) => (
                            <div
                              key={ex.id}
                              className="text-[13px] font-medium text-blue-700 flex justify-between items-center"
                            >
                              <span>
                                + {ex.qty > 1 ? `${ex.qty}x ` : ""}
                                {ex.nombre}
                              ...
                              <span className="text-[11px] text-slate-400">
                                +$
                                {(Number(ex.precio) * ex.qty).toLocaleString(
                                  "es-CO",
                                )}
                              </span>
                            </div>
                      ...
                      <div className="flex items-center justify-between mt-1 pt-2 border-t border-slate-100">
                        ...
                        <div className="flex gap-1.5">
                          <button
                            onClick={() =>
                              setMenuModificador((prev) =>
                                prev.id === item.id && prev.tipo === "extras"
                                  ? { id: null, tipo: null }
                                  : { id: item.id, tipo: "extras" },
                              )
                            }
                            className="flex items-center gap-1 text-[11px] font-medium px-2 py-1.5 rounded-lg transition-colors cursor-pointer bg-white border border-slate-200"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 text-[11px] font-medium px-2 py-1.5 rounded-lg transition-colors cursor-pointer bg-white border border-slate-200"
                          >
                            <Ban className="w-3.5 h-3.5 text-red-500" /> Sin...
                          </button>
                        </div>
                      </div>
                          ...
                          {menuModificador.tipo === "extras" && (
                            <div className="flex flex-wrap gap-2">
                              {extrasDisponibles.map((ext) => {
                                const cantidadExtra = item.extras.filter(
                                  (e) => e.id === ext.id,
                                ).length;
                                ...
                                return (
                                  <div
                                    key={ext.id}
                                    className={`flex items-center gap-1.5 px-2 py-1.5 rounded-lg border transition-colors ${activo ? "bg-blue-600 text-white border-blue-600 shadow-sm" : "bg-white text-slate-700 border-slate-300 hover:border-blue-300"}`}
                                  >
                                    {activo && (
                                      <button
                                        onClick={() =>
                                          quitarExtra(item.id, ext)
                                        }
                                        className="w-5 h-5 flex items-center justify-center rounded-full bg-blue-700 hover:bg-blue-800 text-white font-bold leading-none cursor-pointer pb-0.5"
                                      >
                                        -
                                      </button>
                                    )}
                                    <span
                                      ...
                                      onClick={() => agregarExtra(item.id, ext)}
                                    >
                                      {activo ? `${cantidadExtra}x ` : "+ "}
                                      {ext.nombre}
                                    </span>
                                    <button
                                      onClick={() => agregarExtra(item.id, ext)}
                                      className={`w-5 h-5 flex items-center justify-center rounded-full font-bold leading-none cursor-pointer pb-0.5 ${activo ? "bg-blue-700 hover:bg-blue-800 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
                                    >
                                      +
                                    </button>
                                  </div>
                              ...
                              {INGREDIENTES_REMOVIBLES.map((ing) => {
                                ...
                                return (
                                  <button
                                    key={ing}
                                    onClick={() =>
                                      toggleSinIngrediente(item.id, ing)
                                    }
                                    className={`text-[12px] font-medium px-3 py-1.5 rounded-lg border transition-colors cursor-pointer shadow-sm ${isSelected ? "bg-red-500 text-white border-red-600" : "bg-white text-slate-700 border-slate-300 hover:border-red-300"}`}
                                  >
                                    - {ing}
                                  </button>
                                );
